...
class AgentAgentObsMask2D(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        cone_mask = in_cone2d(
            agent_pos2d, np.squeeze(agent_angle, -1), self.cone_angle, agent_pos2d
        )
        # Make sure they are in line of sight
        for i, j in np.argwhere(cone_mask):
            if i != j:
                if "agent_types" in self.metadata:
                    cone_mask[i, j] = insight(
                        self.unwrapped.sim,
                        self.metadata["agent_geom_idxs"][i],
                        self.metadata["agent_geom_idxs"][j],
                        dist_thresh=self.metadata["agent_types"][i]["view_range"],
                    )
                else:
                    cone_mask[i, j] = insight(
                        self.unwrapped.sim,
...
class AgentSiteObsMask2D(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        cone_mask = in_cone2d(
            agent_pos2d, np.squeeze(agent_angle, -1), self.cone_angle, pos2d
        )
        # Make sure they are in line of sight
        for i, j in np.argwhere(cone_mask):
    ...
    def observation(self, obs):
        ...
        for i, j in np.argwhere(cone_mask):
            ...
            if geom_id == -1:
                ...
                cone_mask[i, j] = insight(
                    self.unwrapped.sim, agent_geom_id, geom2_id=geom_id
                )
        obs[self.mask_obs_key] = cone_mask
        return obs
