...
from mae_envs.wrappers.util import (
...
from mae_envs.wrappers.manipulation import (
    GrabObjWrapper,
    GrabClosestWrapper,
    LockObjWrapper,
    LockAllWrapper,
...
from mae_envs.wrappers.line_of_sight import (
    AgentAgentObsMask2D,
    AgentGeomObsMask2D,
    AgentSiteObsMask2D,
)
from mae_envs.wrappers.prep_phase import (
    PreparationPhase,
    NoActionsInPrepPhase,
    MaskPrepPhaseAction,
)
...
from mae_envs.modules.util import (
    uniform_placement,
    close_to_other_object_placement,
...
class TrackStatWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.in_prep_phase and obs["prep_obs"][0, 0] == 1.0:
            ...
            if self.n_boxes > 0:
                self.max_box_move_prep = np.max(
                    np.linalg.norm(obs["box_pos"] - self.box_pos_start, axis=-1)
                )
                self.num_box_lock_prep = np.sum(obs["obj_lock"])
            if self.n_ramps > 0:
                self.max_ramp_move_prep = np.max(
                    np.linalg.norm(obs["ramp_pos"] - self.ramp_pos_start, axis=-1)
                )
...
def make_env(
    ...
    if n_food > 0:
        ...
        if food_together_radius is not None:
            ...
            food_placement[:: n_food // n_food_cluster] = [
                first_food_placement
            ] * n_food_cluster
        else:
    ...
    if n_food:
        ...
        env = MaskActionWrapper(
            env, "action_eat_food", ["mask_af_obs"]
    ...
    if grab_box and (np.max(n_boxes) > 0 or n_ramps > 0):
        env = GrabObjWrapper(
            env,
            [f"moveable_box{i}" for i in range(np.max(n_boxes))]
