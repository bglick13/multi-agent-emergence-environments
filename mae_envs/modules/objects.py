...
class Boxes(EnvModule):
    ...
    def build_world_step(self, env, floor, floor_size):
        ...
        self.curr_n_boxes = env._random_state.randint(
            self.n_boxes[0], self.n_boxes[1] + 1
        )
        ...
        if self.curr_n_elongated_boxes > 0:
            ...
            self.box_size_array[
                n_xaligned : self.curr_n_elongated_boxes, :
        ...
        for i in range(self.curr_n_boxes):
            ...
            if self.placement_fn is not None:
                ...
                pos, _ = rejection_placement(
                    env, _placement_fn, floor_size, self.box_size_array[i, :2]
                )
                if pos is not None:
    ...
    def modify_sim_step(self, env, sim):
        ...
        self.box_geom_idxs = np.array(
            [
                sim.model.geom_name2id(f"moveable_box{i}")
                for i in range(self.curr_n_boxes)
            ]
        )
        self.box_qpos_idxs = np.array(
            [
                qpos_idxs_from_joint_prefix(sim, f"moveable_box{i}:")
                for i in range(self.curr_n_boxes)
            ]
        )
        self.box_qvel_idxs = np.array(
            [
                qvel_idxs_from_joint_prefix(sim, f"moveable_box{i}:")
                for i in range(self.curr_n_boxes)
            ]
        )
        if self.mark_box_corners:
            self.box_corner_idxs = np.array(
                [
                    sim.model.site_name2id(f"moveable_box{i}_corner{j}")
                    for i in range(self.curr_n_boxes)
                    for j in range(4)
                ]
            )
...
class Ramps(EnvModule):
    ...
    def build_world_step(self, env, floor, floor_size):
        ...
        for i in range(self.n_ramps):
            ...
            if self.placement_fn is not None:
                ...
                pos, _ = rejection_placement(
                    env, _placement_fn, floor_size, get_size_from_xml(geom)
                )
                if pos is not None:
                    floor.append(geom, placement_xy=pos)
                else:
    ...
    def modify_sim_step(self, env, sim):
        # Cache qpos, qvel idxs
        self.ramp_qpos_idxs = np.array(
            [qpos_idxs_from_joint_prefix(sim, f"ramp{i}") for i in range(self.n_ramps)]
        )
        self.ramp_qvel_idxs = np.array(
            [qvel_idxs_from_joint_prefix(sim, f"ramp{i}") for i in range(self.n_ramps)]
        )
        self.ramp_geom_idxs = np.array(
            [sim.model.geom_name2id(f"ramp{i}:ramp") for i in range(self.n_ramps)]
        )
    ...
    def build_world_step(self, env, floor, floor_size):
        ...
        for i in range(self.n_objects):
            ...
            if self.placement_fn is not None:
                ...
                pos, _ = rejection_placement(
                    env, _placement_fn, floor_size, diameter * np.ones(2)
                )
                if pos is not None:
                    floor.append(geom, placement_xy=pos)
                else:
    ...
    def modify_sim_step(self, env, sim):
        if self.make_static:
            self.s_cylinder_geom_idxs = np.array(
                [
                    sim.model.geom_name2id(f"static_cylinder{i}")
                    for i in range(self.n_objects)
                ]
            )
        else:
            self.m_cylinder_geom_idxs = np.array(
                [
                    sim.model.geom_name2id(f"moveable_cylinder{i}")
                    for i in range(self.n_objects)
                ]
            )
            qpos_idxs = [
                qpos_idxs_from_joint_prefix(sim, f"moveable_cylinder{i}")
                for i in range(self.n_objects)
            ]
            qvel_idxs = [
                qvel_idxs_from_joint_prefix(sim, f"moveable_cylinder{i}")
                for i in range(self.n_objects)
            ]
...
class LidarSites(EnvModule):
    ...
    def modify_sim_step(self, env, sim):
        ...
        if isinstance(self.n_lidar_per_agent, list):
            self.lidar_ids = np.array(
                [
                    [
                        sim.model.site_name2id(f"agent{i}:lidar{j}")
                        for j in range(self.n_lidar_per_agent[i])
                    ]
                    for i in range(self.n_agents)
                ]
            )
